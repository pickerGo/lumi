...
export class ImageView extends BaseBlockView {
  ...
  resize = (e) => {
    ...
    // 计算新的宽度
    const currentWidth = this.imageDOM.getBoundingClientRect().width;
  ...
  initEvt = () => {
    ...
    document.addEventListener('mouseup', this.endResize, true);
  ...
  destroy() {
    ...
    document.removeEventListener('mouseup', this.endResize, true);
