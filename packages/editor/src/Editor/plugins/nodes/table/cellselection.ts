...
import { CellAttrs, inSameTable, pointsAtCell, removeColSpan, isInTable } from './util';
...
export class CellSelection extends Selection {
  ...
  public static colSelection(
    ...
    if (anchorRect.top <= headRect.top) {
      if (anchorRect.top > 0)
        $anchorCell = doc.resolve(tableStart + map.map[anchorRect.top * width + anchorRect.left]);
      if (headRect.bottom < map.height)
        $headCell = doc.resolve(
          tableStart +
            map.map[headRect.top * width + headRect.right - 1],
        );
    } else {
      if (headRect.top > 0)
        $headCell = doc.resolve(tableStart + map.map[headRect.top * width + headRect.left]);
      if (anchorRect.bottom < map.height)
        $anchorCell = doc.resolve(
          tableStart +
            map.map[anchorRect.top * width + anchorRect.right - 1],
        );
    }
  ...
  public static rowSelection(
    ...
    if (anchorRect.left <= headRect.left) {
      if (anchorRect.left > 0)
        $anchorCell = doc.resolve(
          tableStart + map.map[anchorRect.top * map.width + anchorRect.left],
        );
      if (headRect.right < map.width)
        $headCell = doc.resolve(
          tableStart + map.map[map.width * (headRect.top) + headRect.left - 1],
        );
    } else {
      if (headRect.left > 0)
        $headCell = doc.resolve(
          tableStart + map.map[headRect.top * map.width + headRect.left]
        );
      if (anchorRect.right < map.width)
        $anchorCell = doc.resolve(
          tableStart + map.map[map.width * (anchorRect.top) + anchorRect.left - 1],
        );
    }
    return new CellSelection($anchorCell, $headCell);
  }
...
export function normalizeSelection(
  ...
  // 如果start是table里， 则处理， 否则不处理。
  if (!isInTable(state)) return tr;
