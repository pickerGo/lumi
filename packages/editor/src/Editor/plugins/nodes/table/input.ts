...
export function handleMouseDown(
  ...
  function stop(): void {
    view.root.removeEventListener('mouseup', stop, true);
    view.root.removeEventListener('dragstart', stop, true);
  ...
  view.root.addEventListener('mouseup', stop, true);
  view.root.addEventListener('dragstart', stop, true);
...
export const handleClick = (
  ...
  // 在td内点击的话， 如果td内容为空， 则添加一个空的textBlock
  const target = event.target as HTMLElement;
  const td = target.closest('td') as HTMLElement;

  if (!td) return;

  // 如果td为空， 则新增一行空行， 并且光标移到新行
