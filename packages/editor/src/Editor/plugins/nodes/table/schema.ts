...
function getCellAttrs(dom: HTMLElement | string, extraAttrs: Attrs): Attrs {
  ...
  const result: MutableAttrs = {
    ...
    colwidth: widths && widths.length == colspan ? widths : [100],
...
function setCellAttrs(node: Node, extraAttrs: Attrs): Attrs {
  ...
  if (node.attrs.background) attrs.style = `background:${node.attrs.background}`;
...
export function tableNodes(options: TableNodesOptions): TableNodes {
  ...
  const cellAttrs: Record<string, AttributeSpec> = {
    ...
    colwidth: { default: [100], validate: validateColwidth },
    background: { default: '', validate: 'string' },
  };
  for (const prop in extraAttrs)
    cellAttrs[prop] = {
  ...
  return {
    table: {
      ...
      create(schema: Schema, attrs: any) {
        const id = nanoid();
        ...
        return schema.nodes['table'].create({
          ...
          schema.node('table_row', { id: nanoid(8) }, [
            schema.node('table_cell', { id: nanoid(8), colwidth: [100] }, [
