...
export default defineComponent({
    ...
    setup(props) {
        ...
        return () => (
            <div class={[props.active ? 'active' : '']} onClick={() => handleAction(schema.marks.italic)}>
                <Tooltip title={i18next.t('editor.commands.italic')}>
                    <LucideIcon icon={Italic} width={props.iconSize}></LucideIcon>
                </Tooltip>
            </div>
        );
    },
