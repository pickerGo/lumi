...
export default defineComponent({
    ...
    setup(props) {
        ...
        return () => (
            <div class={[props.active ? 'active' : '']} onClick={() => handleAction(schema.marks.strikethrough)}>
                <Tooltip title={i18next.t('editor.commands.strike')}>
                    <LucideIcon icon={Strikethrough} width={props.iconSize}></LucideIcon>
                </Tooltip>
            </div>
        );
    },
